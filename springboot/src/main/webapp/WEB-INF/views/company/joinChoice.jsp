...
<ul class="nav nav-pills nav-justified mb-3" id="ex1" role="tablist" >
  <li class="nav-item" role="presentation" style="height: 60px;" >
    <a class="nav-link active" id="mem-register-form-link" href="#" style="font-size: 25px;">개인 일반회원</a>
  </li>
  <li class="nav-item" role="presentation" style="height: 60px;">
    <a type="" class="nav-link" id="company-register-form-link"  href="#" style="font-size: 25px;" >사업자 병원회원</a>
  </li>
</ul>
</div>
<!-- Pills navs -->
...
<div class="tab-content">
    <form id="mem-register-form" role="form">
      <div class="text-center mb-3">
        <h3 style="height: 53px; margin-top: 36px;">Ateam 홈페이지에 오신것을 환영합니다.</h3>
        <h5 style="height: 25px;">지금 회원 가입하신 후 Allcare의 다양한 서비스를 만나보세요.</h5>
      </div>
    ...
    <form id="company-register-form" method="post" action="${pageContext.request.contextPath }/admin/caInsert" role="form">
      ...
      <div class="form_box">
	      <div class="text-center mb-3">
	        <h3 style="height: 53px; margin-top: 36px;">Allcare 홈페이지에 오신것을 환영합니다.</h3>
		...
		<div class="col-md-4" style="margin-right: auto; margin-left: 280px; margin-top: 10px;" >
			<div class="form-check form-check-inline">
			  <label class="form-check-label" for="ccate">병원등록</label>
			  <input class="form-check-input" type="radio" name="ccatev" id="ccatev" value="병원" checked="checked">
			</div>	
			<div class="form-check form-check-inline">
			  <label class="form-check-label" for="ccate">약국등록</label>
			  <input class="form-check-input" type="radio" name="ccatev" id="ccatev" value="약국" >
			</div>	
		</div>	      	  
	...
	// db에 사업자등록번호와 중복되는지 확인
		...
		$js.ajax({
			...
			success:function(data){
				console.log(data);
				if(data == 1){
					$js('#confcrn').css('color','red').html('사이트에 등록된 사업자번호입니다.');
					$js('#crnBtn').attr('disabled', true);
				}else{
					$js('#confcrn').css('color','green').html('사이트에 등록지 않은 사업자번호입니다.');
...
/* 	$('input[type="checkbox"][name="ccate"]').click(function() {
	...
	$.ajax({
	  url: "https://api.odcloud.kr/api/nts-businessman/v1/status?serviceKey="+sKey,  // serviceKey 값을 xxxxxx에 입력
	  type: "POST",
	  data: data,
		 ...
		 // console.log("타입"+JSON.stringify(data_1));
	      ...
	      if(result.data[0].tax_type === "국세청에 등록되지 않은 사업자등록번호입니다."){
	    	  if(result.data[0].b_no === "3988701111"){ // 가짜 사업등록번호
		    	  ...
		    	  $('#confcrn').css('color','red').html('X 국세청에 등록되지 않은 사업자등록번호입니다.');
		    	  let reset = "";
		    	  $('#crnv').val(reset);
		    	  $js('#crnBtn').attr('disabled', true);
		    	  return false;
